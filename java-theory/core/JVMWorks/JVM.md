...
JVM (Java Virtual Machine) - это виртуальная машина Java, которая является частью исполнительной среды Java (JRE - Java
Runtime Environment). Она предназначена для выполнения Java-программ, представленных в виде байткода. JVM является
абстрактной машиной, что означает, что она предоставляет среду выполнения для Java-программ без привязки к конкретной
аппаратной или программной платформе.

Основные функции JVM включают загрузку, верификацию, интерпретацию и выполнение байткода Java, а также управление
памятью и потоками выполнения. Она также обеспечивает множество других возможностей, таких как динамическая загрузка
классов, оптимизация выполнения кода и взаимодействие с операционной системой и другими приложениями через Java Native
Interface (JNI).

### За что отвечает JVM :

1. Загрузка, проверка и выполнение байт кода;
2. Предоставление выполнения условия для выполнения байт-кода;
3. Управление памятью и очисткой мусора (Сборка мусора);
...
машина. Типы long и double, которые являются 64-битными, поддерживаются изначально, но занимают две единицы памяти в
frame's local или стеке операндов, поскольку каждая единица составляет 32 бита. Типы boolean, byte, short и char имеют
...
Существует три типа ссылочных типов: типы классов, типы массивов и типы интерфейсов. Их значения являются ссылками на
динамически создаваемые экземпляры классов, массивы или экземпляры классов, которые реализуют интерфейсы соответственно.
